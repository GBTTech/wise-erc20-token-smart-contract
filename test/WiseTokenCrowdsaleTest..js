...
contract('WiseTokenCrowdsale', function([_, wallet, investor1, investor2,investor3,  foundersFund, foundationFund, partnersFund]) {
  ...
  describe('minted crowdsale', function() {
    it('mints tokens after purchase', async function() {
      ...
      await this.crowdsale.sendTransaction({ value: ether(eth), from: investor1 });
      const newTotalSupply = await this.token.totalSupply(); 
      assert.isTrue(newTotalSupply > originalTotalSupply);
      assert.isTrue(newTotalSupply.toNumber() === ether(eth).toNumber());
    });
  });
  ...
  describe('refundable crowdsale', function() {
    ...
    describe('when the corwdsale stage is PrivateICO', function() {
      beforeEach(async function () {
        const balance = await web3.eth.getBalance(this.wallet);  
        await this.crowdsale.setCrowdsaleStage(this.privateIcoStage, { from: _ });
        await this.crowdsale.buyTokens(investor1, { value: ether(1), from: investor1 });
      });

      it('forwards funds to the wallet', async function () {
        const currentBalance = 103000000000000000000;
        const expetedBalance = 104000000000000000000;
        const balance = await web3.eth.getBalance(this.wallet);
        expect(balance.toNumber()).to.be.above(ether(100).toNumber());
        expect(balance.toNumber()).to.be.eq(expetedBalance);
      });
    });

    describe('when the corwdsale stage is PrieICO', function() {
      beforeEach(async function () {
      ...
      it('forwards funds to the wallet', async function () {
        const currentBalance = 104000000000000000000;
        const expetedBalance = 106000000000000000000;
        const balance = await web3.eth.getBalance(this.wallet);  
        expect(balance.toNumber()).to.be.above(ether(100).toNumber());
        expect(balance.toNumber()).to.be.eq(expetedBalance);
      });
  ...
  describe('crowdsale stages', function() {
    ...
    it('change to private ICO first time', async function () {
      await this.crowdsale.setCrowdsaleStage(this.privateIcoStage, { from: _ });
      const stage = await this.crowdsale.stage();
      stage.should.be.bignumber.equal(this.privateIcoStage);
      const rate = await this.crowdsale.rate();
      rate.should.be.bignumber.equal(this.privateIcoRate);
    }); 

    it('change to pre ICO ', async function () {
      await this.crowdsale.setCrowdsaleStage(this.preIcoStage, { from: _ });
      const stage = await this.crowdsale.stage();
    ...
    it('change to public ICO', async function () {
      await this.crowdsale.setCrowdsaleStage(this.icoStage, { from: _ });
      const stage = await this.crowdsale.stage();
  ...
  describe('buyTokens()', function() {
    describe('when the contribution is less than the minimum cap', function() {
      it('rejects the transaction', async function() {
        const value = this.investorMinCap / 2;
  ...
  describe('when the total contributions exceed the investor hard cap', function () {
    it('rejects the transaction', async function () {
      ...
      const value2 = this.inestorHardCap * 2;
  ...
  describe('finalizing the crowdsale', function() {
    ...
    describe('when the goal is reached', function() {
      beforeEach(async function () {
        ...
        await this.crowdsale.buyTokens(investor3, { value: ether(5), from: investor3 }); 
